...
# walk up two polygons to determine the points to merge together
# to create a larger convex hull

def walkUpward(l,r):
    countLeft = 0
    countRight = 0
    # walk upward algorithm from class notes
    while turn(l.ccwPoint, l, r) == LEFT_TURN or turn(l,r,r.cwPoint) == LEFT_TURN:
        if turn(l.ccwPoint, l, r) == LEFT_TURN:
            # this if statement is to remove points not on hull
            # if the count is 0, we need the keep cwPoint for
            # use when walkDownward is called
            # if it's any other point though, remove both cw and cww
            # theory is the same for the other half of walkUpward
            # and both halves of walkDownward
            if countLeft > 0:
                temp = l.ccwPoint
                l.ccwPoint = None
                l.cwPoint = None
...
# walk down two polygons to determine the points to merge together
# to create a larger convex hull

def walkDownward(l,r):
    countLeft = 0
    countRight = 0
    # walk downward algorithm derived from altering walkUpward
    while turn(r.ccwPoint, r, l) == LEFT_TURN or turn(r,l,l.cwPoint) == LEFT_TURN:
        if turn(r.ccwPoint, r, l) == LEFT_TURN:
            if countRight > 0:
                temp = r.ccwPoint
                r.ccwPoint = None
                r.cwPoint = None
...
def hullMerge(first_hull, second_hull):
    allPoints = first_hull + second_hull

    leftTopTemp = first_hull[-1] # rightmost point of left hull
    rightTopTemp = second_hull[0] # leftmost point of right hull

    leftBotTemp = first_hull[-1] # rightmost point of left hull
    rightBotTemp = second_hull[0] # leftmost point of right hull

    leftTop, rightTop = walkUpward(leftTopTemp,rightTopTemp)

    leftBot, rightBot = walkDownward(leftBotTemp,rightBotTemp)


    leftTop.cwPoint = rightTop
    rightTop.ccwPoint = leftTop

    leftBot.ccwPoint = rightBot
    rightBot.cwPoint = leftBot

    # hullMerge returns a list of points on the new merged hull

    hull = []

    for point in allPoints:
        if point.cwPoint != None and point.ccwPoint != None:
            hull.append(point)
...
def buildHull( points ):
    ...
    # print(points) # commented out, for debugging
    ...
    if len(points) == 3:
        ...
        # if statement to make sure cw and ccw
    ...
    left = buildHull(points[:len(points)//2])
    right = buildHull(points[(len(points)//2):])
    display()
    return hullMerge(left,right)
